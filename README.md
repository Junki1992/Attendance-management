...
- シフトは **15日ごと** に提出。
- **1～15日分**: 前月の指定日・時刻までに提出（デフォルト: 前月25日 23:59）。**16日～月末分**: 当月の指定日・時刻まで（デフォルト: 当月10日 23:59）。締切を過ぎた日はスタッフは編集不可。
- 管理者は **設定** ページで「シフト提出締切」の日付・時刻を変更でき、土日祝でずれる月は **月別上書き** で締切日時を指定できる。
- 催促は GitHub Actions で **毎日 09:00 JST** に実行され、その日が締切日に当たる場合に未提出者へアプリ内通知を送信（設定の締切日・月別上書きに従う。Blaze プラン不要・Secrets `GOOGLE_APPLICATION_CREDENTIALS_JSON` を利用）。
