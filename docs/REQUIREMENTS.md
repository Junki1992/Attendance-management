...
| 2 | シフト**確定**→アルバイトへ**自動通知**＋**未読/既読**表示 | 確定時に通知送信あり。既読は `read` で実装済み、管理画面での一覧表示はなし | 管理者側「通知一覧＋未読/既読」UI |
| 3 | **36協定アラート**：1日8h / 週40h 超過でアラート | 管理者シフト表で 8h/40h 超過は色分け・⚠️表示済み | 目立つアラート枠・一覧、必要なら音 or バッジ |
| 4 | シフト**確定後**の**変更申請**→管理者承認→シフト表へ自動反映 | なし | 変更申請スキーマ、申請/承認フロー、承認時にシフト更新 |
| 5 | 締切後は**修正不可** or 締切**後の修正は赤字**等で表示 | なし | 締切日チェック、スタッフ編集制限、管理者表で「締切後編集」フラグ＋赤字 |
